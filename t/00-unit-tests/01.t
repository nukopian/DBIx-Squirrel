...
sub test_the_basics {
    ...
    ($exp, $got) = (
        [undef, 'SELECT * FROM table WHERE col = ?', get_trimmed_sql_and_digest('SELECT * FROM table WHERE col = ?'),],
        do {
    ...
    ($exp, $got) = (
        [{1 => '$1',}, 'SELECT * FROM table WHERE col = ?', get_trimmed_sql_and_digest('SELECT * FROM table WHERE col = $1'),],
        do {
    ...
    ($exp, $got) = (
        [{1 => ':n',}, 'SELECT * FROM table WHERE col = ?', get_trimmed_sql_and_digest('SELECT * FROM table WHERE col = :n'),],
        do {
    ...
    ($exp, $got) = (undef, DBIx::Squirrel::st::_placeholders_are_positional(undef),);
    is_deeply $got, $exp, '_placeholders_are_positional'
      or dump_val {exp => $exp, got => $got};

    ($exp, $got) = (undef, DBIx::Squirrel::st::_placeholders_are_positional({1 => ':name',}),);
    is_deeply $got, $exp, '_placeholders_are_positional'
      or dump_val {exp => $exp, got => $got};
    ...
    ($exp, $got) = ({1 => ':1',}, DBIx::Squirrel::st::_placeholders_are_positional({1 => ':1',}),);
    is_deeply $got, $exp, '_placeholders_are_positional'
      or dump_val {exp => $exp, got => $got};
    ...
    ($exp, $got) = ([], [DBIx::Squirrel::st::_map_placeholders_to_values(undef)],);
    is_deeply $got, $exp, '_map_placeholders_to_values'
      or dump_val {exp => $exp, got => $got};

    ($exp, $got) = (['a', 'b'], [@{DBIx::Squirrel::st::_map_placeholders_to_values(undef, ('a', 'b'),)}],);
    is_deeply $got, $exp, '_map_placeholders_to_values'
      or dump_val {exp => $exp, got => $got};

    ($exp, $got) = (
        {'?1' => 'a', '?2' => 'b'},
        {@{DBIx::Squirrel::st::_map_placeholders_to_values({1 => '?1', 2 => '?2'}, ('a', 'b'),)}},
    );
    is_deeply $got, $exp, '_map_placeholders_to_values'
      or dump_val {exp => $exp, got => $got};
    ...
    ($exp, $got) = (
        {':n1' => 'a', ':n2' => 'b'},
        {@{DBIx::Squirrel::st::_map_placeholders_to_values({1 => ':n1', 2 => ':n2'}, (':n1' => 'a', ':n2' => 'b'),)}},
    );
    is_deeply $got, $exp, '_map_placeholders_to_values'
      or dump_val {exp => $exp, got => $got};
    ...
    $sth = $standard_ekorn_dbh->prepare(join ' ', ('SELECT *', 'FROM media_types', 'WHERE Name = ?',),);

    ($exp, $got) = ({1 => 'AAC audio file'}, {$sth->bind_param(1 => 'AAC audio file')},);
    is_deeply $got, $exp, 'bind_param'
      or dump_val {exp => $exp, got => $got};

    $sth = $standard_ekorn_dbh->prepare(join ' ', ('SELECT *', 'FROM media_types', 'WHERE Name = ?1',),);
    ...
    $sth = $standard_ekorn_dbh->prepare(join ' ', ('SELECT *', 'FROM media_types', 'WHERE Name = :name',),);

    ($exp, $got) = ({1 => 'AAC audio file'}, {$sth->bind_param(':name' => 'AAC audio file')},);
    is_deeply $got, $exp, 'bind_param'
      or dump_val {exp => $exp, got => $got};
    ...
    ($exp, $got) = (bless({MaxRows => 10, Slice => [],}, 'DBIx::Squirrel::it'), $it,);
    ...
    $sth = $standard_ekorn_dbh->prepare(join ' ', ('SELECT *', 'FROM media_types',),);
    ...
    ($exp, $got) = ([1, 'MPEG audio file'], $it->first,);
    ...
    ($exp, $got) = ({MediaTypeId => 1, Name => "MPEG audio file"}, $it->first({}),);
    is_deeply $got, $exp, 'first'
      or dump_val {exp => $exp, got => $got};

    ($exp, $got) = ({MediaTypeId => 2, Name => "Protected AAC audio file"}, $it->next,);
    is_deeply $got, $exp, 'next'
      or dump_val {exp => $exp, got => $got};

    $sth->finish;

    $sth = $standard_ekorn_dbh->prepare(join ' ', ('SELECT *', 'FROM media_types', 'WHERE MediaTypeId = :id',),);

    $it = $sth->it(id => 1);

    ($exp, $got) = ([1, 'MPEG audio file'], $it->single,);
    is_deeply $got, $exp, 'single'
      or dump_val {exp => $exp, got => $got};

    ($exp, $got) = ([5, 'AAC audio file'], $it->single(id => 5),);
    is_deeply $got, $exp, 'single'
      or dump_val {exp => $exp, got => $got};
    ...
    my $sth = $standard_ekorn_dbh->prepare(join ' ', ('SELECT *', 'FROM media_types',),);
    ...
    ($exp, $got) = (bless({MaxRows => 10, Slice => []}, 'DBIx::Squirrel::rs'), $sth->rs->_maxrows(10),);
    is_deeply $got, $exp, 'results'
      or dump_val {exp => $exp, got => $got};

    ($exp, $got) = (bless([1, "MPEG audio file"], 'DBIx::Squirrel::rs'), $sth->rs->first,);
    is_deeply $got, $exp, 'first'
      or dump_val {exp => $exp, got => $got};
    ...
    ($exp, $got) = (
        ...
        do {
            $sth->finish;
            $sth = $standard_ekorn_dbh->prepare('SELECT MediaTypeId, Name FROM media_types',);
    ...
    diag "$_\n"
      for $standard_ekorn_dbh->rs('SELECT MediaTypeId, Name FROM media_types', sub {$_->Name}, sub {"Media type: $_"},)
      ->all;
