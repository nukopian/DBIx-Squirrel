...
The C<prepare> method works just the same as it does in the C<DBI>.

The C<$statement> argument must evauate to a string, and may be any of the
following:
...
C<DBIx::Squirrel> isn't too opinionated about the placeholder style used,
and will accept C<:name>, C<:number>, C<$number>, C<?number> or C<?>
...
placeholders within the statement-string. You must use key-value bindings
if you opted for named placeholders. All statements are normalised to use
the legacy C<?> style anyway, and it is this normalised statement that is
cached by the C<DBI>. 
...
The C<DBIx::Squirrel::it> package exports a number of subroutines that
